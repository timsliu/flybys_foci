...
class spacecraft():
	...
	def long_burn(self, dv, time, steps, delta):
		...
		inputs: dv - total delta-v (km/s) of the burn
		        ...
		        steps - number of discrete burns to break the long
		                burn into
		        delta - size of delta for approximating slope'''
	...
	def coast_time(self, coast_period, delta = 1000):
	...
	def coast_distance(self, n, x_final):
		...
		inputs:  x_final - final distance to coast to (max self.dis_travel) (AU)
                 n       - number of points to use for Simpson's approximation     
        ...
        f_values = [1/calc_v_2(self.v0, self.r, r_values[x]) 
                   for x in range(n)]
