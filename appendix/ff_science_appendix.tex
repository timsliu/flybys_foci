...
In the story \textit{Einstein} takes ``decades" to reach the sun's focal point. This section  calculates how long the trip takes based on the specific impulse of the \textit{Sheridan} drive and the distance to the focal point.
...
The burn at perihelion is the \textit{Sheridan} drive's longest and most powerful burn. In the story the engine burns continuously for several days, accelerating \textit{Einstein} out of the solar system. For our calculations we will treat the burn as if it were much faster and the change in velocity was instantaneous. This is an idealized burn and gives us a higher v-infinity than a burn spread out over several days. But we'll stick with this approximation because it's not \textit{that} far off from a longer burn.

Using equations for Section 1 the velocity at perihelion before the burn is 

$$v_{p-} = $$

where the $p-$ subscript denotes the velocity prior to the burn. \textit{Einstein} uses the majority of its fuel close to perihelion so the change in velocity is whatever is left of its delta-v budget:

$$v_{p+} = $$

After the burn \textit{Einstein} is 0.1AU from the sun and is traveling at xx m/s, or yy the speed of light. Solving the equation for v-infinity from section 1 gives:
